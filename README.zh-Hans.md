...
NeoDB 实例、session 密钥、OAuth 重定向来源、可选的 TMDB、Google Books 与 Azure 翻译凭据、
SEO/索引配置，以及可选的出站代理。**密钥只应放在 `.env.local` 中**，该文件已被 `.gitignore` 排除。

`GOOGLE_BOOKS_API_KEY` 为可选配置。当 NeoDB 中缺少对应版本的图书总页数时，它会为阅读进度
提供备用页数。未配置时，用户仍可手动填写总页数或总章节数。
