    ...
    <main className="min-h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      ...
      <FloatingTopBar className="fixed inset-x-0 top-0 z-40" rowClassName="max-w-4xl justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {!isNoteEditor ? (
            <button
              className="inline-flex h-9 items-center rounded-full border border-white/70 bg-white/50 px-3 text-sm font-bold text-[#333e50] shadow-sm transition hover:bg-white/75 press-control"
          ...
          <button
            className="rounded-full bg-[var(--theme-primary)] px-5 py-2 text-sm font-bold text-white shadow-sm transition hover:bg-[var(--theme-primary-hover)] press-control disabled:cursor-wait disabled:bg-[#c1c7cf]"
...
function ToolbarButton({
  ...
  return (
    <button
      aria-label={label}
      className="relative grid size-10 place-items-center rounded-full text-[#44474c] transition hover:bg-white/60 hover:text-[var(--foreground)] press-icon sm:size-12"
      onClick={onClick}
      onMouseDown={(event) => event.preventDefault()}
      title={label}
      type="button"
